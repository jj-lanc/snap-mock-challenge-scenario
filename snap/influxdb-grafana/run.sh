...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/service_dashboard.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/fr_status.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/fr_hardware.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/dc2.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/dc3.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/rec_status.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/ls_status.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/auth_perf.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
echo -n "${green}starting snap daemon${reset}"
$SNAP_PATH/bin/snapd -c 4 -m 10 -l 5 -t 0 --config  $SNAP_PATH/cfg/config.json &
echo ""

sleep 3

echo "${green}loading snap-plugin-publisher-influxdb${reset}"
($SNAP_PATH/bin/snapctl plugin load $SNAP_PATH/plugin/snap-plugin-publisher-influxdb) || die "Error: failed to load influxdb plugin"

echo "${green}loading snap-plugin-collector-exec${reset}"
($SNAP_PATH/bin/snapctl plugin load $SNAP_PATH/plugin/snap-plugin-collector-exec) || die "Error: failed to load psutil plugin"

# echo -n "${greeN}adding task${reset}"
#TMPDIR=${TMPDIR:="/tmp"}
#TASK="${TMPDIR}/snap-task-$$.json"
#echo "$TASK"
#cat $SNAP_PATH/tasks/exec-influx.json | sed s/INFLUXDB_IP/${dm_ip}/ > $TASK
$SNAP_PATH/bin/snapctl task create -t $SNAP_PATH/tasks/default.json
