...
SNAP_PATH=/home/prowess/Desktop/loadgen/snap/
export SNAP_PATH
...
dashboard=$(cat $SNAP_PATH/influxdb-grafana/grafana/test.json)
curl --cookie "$COOKIEJAR" \
	-X POST \
	--silent \
	-H 'Content-Type: application/json;charset=UTF-8' \
...
$SNAP_PATH/bin/snapd --log-level 1 -t 0 --config $SNAP_PATH/cfg/config.json > /tmp/snap.out 2>&1  &
echo ""
